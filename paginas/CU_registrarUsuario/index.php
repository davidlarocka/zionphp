...
//=================================================================
	...
	$psecreta=$_POST["p_secreta"];
		...
		$objUsuario=$consultasCU_registrarUsuario->verificarUsuario($login);
		if($objUsuario[0][0]!=""){
			
			$mensaje.="<div align='center'><b>¡Alerta!</b></br></br><font color='#BC0D0D'>Esta cédula pertenece a un usuario ya registrado</font></div>";
			
		}else{
		
		//$objUsuarioRegistrar=$consultasCU_registrarUsuario->registrarusuario($cedula, $password, $p_nombre, $p_apellido, $seccion, $carrera  );
		$objUsuarioRegistrar=$consultasCU_registrarUsuario->registrarusuario($login, $nombres, $apellidos, $cedula, $password, $correo, $psecreta);
	...
	//llenamos el mensaje que estara dentro de nuestro marco html de salida	
		...
		$mensaje='<div align="center"><b><i>Ingresa Tus Datos</i></b></div>
					<form name="form_login" id="form_login" action="" method="post">
						<table class="tabla" border="0" bgcolor="#F1F1F1">
