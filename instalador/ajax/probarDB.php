	...
	if($motor=="mysql"){	
		...
		if($estadoCliente==true){
			...
			if($link==false){
					...
					echo "Conexion Exitosa <br/><p align='center'><input type='button' value='Instalar base de datos' onclick=\" crearDB(server.value, user.value, pass.value, nombre_bd.value, gestor_bd.value, nombre_sis.value, acronimo.value, admin.value, clave_admin.value)\" /></p>
		";	
			}
