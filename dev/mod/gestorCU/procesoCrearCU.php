<?php 
	...
	if($ajax=="true"){
		//creamos la carpeta
		$rutaCrear="../../../paginas/CU_".$nombreCU."/ajax";
    	mkdir($rutaCrear,0777)or die ("no se pudo crear el modulo"); 
	    chmod($rutaCrear,0777)or die ("so se pudieron dar privilegios publicos");
		
		//creamos el fichero que contendra el objeto ajax con las clases para realizar las consultas de ese caso de uso
		$rutaCrearFichero=$rutaCrear."/ajax.php";
		$gestor = fopen($rutaCrearFichero, "w") or die ("no se crearon los archivos de Reportes"); 
		// escribimos el script php
		fwrite($gestor, $contenidoAjax); 
		fclose($gestor); 	
		chmod ($rutaCrearFichero,0777); 
	}
	...
	$url="../CU_".$nombreCU;
	$acceso=1;
	$nombreCU="CU_".$nombreCU;
	$objMenusExistentes	= $consultasMenus->crearMenus($nivel, $id_menu_padre, $descripcion, $url, $acceso, $nombreCU);
